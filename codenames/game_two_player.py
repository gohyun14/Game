...
class GameTwoPlayer:
    ...
    def __init__(self, red_codemaster, red_guesser, blue_codemaster, blue_guesser,
        ...
        self.red_codemaster = red_codemaster(**cm_kwargs)
        self.red_guesser = red_guesser(**g_kwargs)
        self.blue_codemaster = blue_codemaster(**cm_kwargs)
        self.blue_guesser = blue_guesser(**g_kwargs)
        self.winner = None
    ...
    def get_key_grid(self, color):
        """Return the codemaster's key"""
        if color == "Red":
          return self.key_grid
        self.key_grid_reversed = []
        print(f"key grid: {self.key_grid}")
        print(f"key grid reversed: {self.key_grid_reversed} type: {type(self.key_grid_reversed)}")
        for i in range(len(self.key_grid)):
          if self.key_grid[i] == "Blue":
            self.key_grid_reversed.append("Red")
          elif self.key_grid[i] == "Red":
            self.key_grid_reversed.append("Blue")
          else:
            self.key_grid_reversed.append(self.key_grid[i])
        return self.key_grid_reversed

    def _accept_guess(self, guess_index, color):
        ...
        if self.key_grid[guess_index] == color:
            self.words_on_board[guess_index] = f"*{color}*"
            if self.words_on_board.count(f"*{color}*") >= (8 if color == "Red" else 7):
                return GameCondition.WIN
            return GameCondition.HIT_GOOD
    ...
    def write_results(self, num_of_turns):
        ...
        with open("results/bot_results_new_style_two_player.txt", "a") as f:
    ...
    def run(self):
        ...
        game_condition = GameCondition.HIT_GOOD
        game_counter = 0
        current_player = "Red"
        while game_condition != GameCondition.LOSS and game_condition != GameCondition.WIN:
            ...
            current_key_grid = self.get_key_grid(current_player)
            self._display_key_grid()
            self._display_board_codemaster()

            # assign codemaster and guesser to correct color
            if current_player == "Blue":
                codemaster = self.blue_codemaster
                guesser = self.blue_guesser
            if current_player == "Red":
                codemaster = self.red_codemaster
            ...
            game_condition = GameCondition.HIT_GOOD
            while guess_num <= clue_num and keep_guessing and game_condition == GameCondition.HIT_GOOD:
                ...
                game_condition = self._accept_guess(guess_answer_index, current_player)

                if game_condition == GameCondition.HIT_GOOD:
                    print('\n' * 2)
                    self._display_board_codemaster()
                    guess_num += 1
                ...
                # if guesser selected a civilian or a blue-paired word
                    ...
                    self.winner = "Blue" if current_player == "Red" else "Red"
